...
namespace Snitch.Client
{
    public class WebClient : IWebClientAsync, IDisposable
  {
    ...
    public WebClient(IWebClientOptions options, ILogger Logger, ObservableCollection<string> messages)
    {
      this.Logger = Logger;
      this.Options = options;
      this.SessionId = Guid.NewGuid().ToString();
    ...
    public event DisconnectedEventler OnDisconnectedAsync;
        ...
        {
            using (this.Logger.BeginScope<string>(LoggerHelper.GetCaller(nameof(SendMessageAsync))))
            {
                try
                {
                    var sendMessageData = new GetMaessageRequest
                    {
                        Message = Message
                    };

                    var sendMessageWebResponse = await this.SendRequestAsyncControl(
                        BinaryConverter.Serialize(sendMessageData),
                        sendMessageData.GetType().Name,
                        TimeSpan.FromSeconds(5));

                    if (sendMessageWebResponse.Exception != null)
                        throw pingWebResponse.Exception;

                    var sendMessageResponse = BinaryConverter.Deserialize<GetMaessageResponse>(sendMessageWebResponse.Data);

                    if (!string.IsNullOrWhiteSpace(sendMessageResponse.ErrorString))
                        throw new Exception("|OS|" + sendMessageResponse.ErrorString);
